...
$(function() {
    ...
    function statistics_click(e) {
        ...
        var uid = zGetUid();
        var pId = "";
        var url = zProtocol + logDomain + "/click.gif?key=" + key + "&hf=" + hf
                  + "&vi=" + vi
                  + "&tc=" + tc
...
function statise_view(domain,t,ids,vi,ti) {
    ...
    var uid = zGetUid();
    var image = new Image()
    image.src = zProtocol + domain + "/view.gif?t="+t+"&ids="+ids+"&vi="+vi+"&ti="+ti+"&zid="+zid+"&uid="+uid;
}
...
function statise_show(domain,type,id,fromVersion) {
    ...
    image.src = zProtocol + domain + "/show.gif?ver=1.1&type="+type+"&key="+id+"&t="+date+"&from="+fromVersion+"&uid="+uid+"&zid="+zid;
}
...
(function(){function g(a){if(window.sessionStorage)return window.sessionStorage.getItem(a);return null}function f(a,b){var c=new Image,d="zcool_log_"+Math.floor(Math.random()*2147483648).toString(36);window[d]=c;c.onload=c.onerror=c.onabort=function(){c.onload=c.onerror=c.onabort=null;c=window[d]=null;b&&b(a)};c.src=a}function h(){this.valueStack={};this.beginAnalyse()}var k=navigator.cookieEnabled,l=navigator.javaEnabled(),m=navigator.language||navigator.browserLanguage||navigator.systemLanguage||
    ...
    this.sendLogToServer(zProtocol + logDomain + "/page.gif?")}catch(a){var b=[];b.push("n="+encodeURIComponent(a.name));b.push("m="+encodeURIComponent(a.message));b.push("r="+encodeURIComponent(document.referrer));f(zProtocol + logDomain +"/"+b.join("&"))}},checkHostName:function(a,b){a=a.replace(/:\d+/,"");b=b.replace(/:\d+/,"");pos=a.indexOf(b);return pos>-1&&pos+b.length==a.length},getCookieDomain:function(){var a=window.location.hostname;for(i=0;i<e.length;i++)if(this.checkHostName(hostName,e[i]))return e.replace(/(:\d+)?[\/\?#].*/,
